...
class ModelTrainer:
    ...
    def evaluate(self, step, log_writer, data_reader, sess):
        count_correct = 0
        count_total = 200
        ...
        min_loss_5 = min_5[0]
        min_index5 = min_5[1]
        log_writer.write('=====> step {0}, begin evaluation'.format(step))

        for i in range(count_total):
            eval_data_batch = data_reader.next_batch_evaluation()
            ...
            if 0 in min_index5_:  # top5 for evaluation
                ...
                log_writer.write('Correct prediction' + '\n')
            else:
                print('index %d false predict' % i)
                log_writer.write('False prediction' + '\n')

            q_vec = b_query_vec[0]
            q_name=data_reader.devectorize(q_vec, '')
            log_writer.write(' '.join(q_name) + '\n')
            entity_name = []
            if 0 not in min_index5_:
                ...
                for idx in min_index5_:
                    if idx != 0:
                        ...
                        ent_vec = b_entity_vec[0]
                        true_ent_name = data_reader.devectorize(ent_vec, '')
                        true_ent_name = list(' '.join(true_ent_name))
                        entity_name.append(true_ent_name)
            entity_name = '\t'.join([''.join(name) for name in entity_name])
    ...
    def train(self, data_processor):
        ...
        with tf.Session(config=config) as session:
            ...
            precision = self.evaluate(step, log_writer, data_processor, session)
            if precision>0.5:
                print('\ngood initiation point =====> begin to save checkpoint, step = {0} precision = {1}'.format(step,
                                                                                                       precision))
                path = saver.save(session,
                                  self.output_model_path + '/step' + str(step) + '_precision' + '{0:.4f}'.format(
                                      precision) )
                print("Save checkpoint(model) to {}".format(path))

            while step < FLAGS.num_epochs * 4269 / FLAGS.train_batch_size:  # 13176
                ...
                train_q_vec_b, train_q_vec_len_b, train_d_vec_b, train_d_vec_len_b, train_label_b = train_data_batch
                feed_dict = {
                    train_q_vec: train_q_vec_b,
                    train_q_vec_len: train_q_vec_len_b,
                    train_d_vec: train_d_vec_b,
                    train_d_vec_len: train_d_vec_len_b,
                    train_label: train_label_b}
                _, loss_sum, loss_avg, weight, step_, summary, step = session.run([train_op, train_loss, avg_train_loss, self.total_weight, inc_step, summary_op, global_step], feed_dict)
                print('step = {0}, loss = {1}, avg_loss = {2:.4f}'.format(step, loss_sum, loss_avg))
                # log performance
                if step % self.log_frequency == 0:
                    ...
                    print(log_format_str % (datetime.now(), step, loss_sum, loss_avg, examples_per_sec, sec_per_batch, weight))

                # metrics evaluation and checkpoint model saving
                if step % self.checkpoint_frequency == 0:
                    precision = self.evaluate(step, log_writer, data_processor, session)
                    log_writer.write('step = {0},\tprecision = {1}, loss_avg = {2}'.format(step, precision, loss_avg))
                    log_writer.flush()
                    ...
                    if impr:
                        ...
                        print('\n improved =====> begin to save checkpoint, step = {0} precision = {1}'.format(step,
                                                                                                               precision))
                        path = saver.save(session,
                                          self.output_model_path + '/step' + str(step) + '_precision' + '{0:.4f}'.format(
                                              precision) + '_lossavg' + '{0:.4f}'.format(loss_avg))
                        print("Save checkpoint(model) to {}".format(path))
                        ...
                        query_cnt = 162
                        print('not improved =====> best_precision at step = {0} precision = {1}'.format(self.metrics.best_step, self.metrics.best_value))
